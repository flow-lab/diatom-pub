...
// DBStatusCheck returns nil if it can successfully talk to the database. It
// returns a non-nil error otherwise.
func DBStatusCheck(ctx context.Context, db *pg.DB) error {
	var num int
	_, err := db.WithContext(ctx).QueryOne(pg.Scan(&num), "SELECT 1+1")
	return err
}
